  ...
  if ($("#ResponsableText2").val().trim() == "") {
    var claseObj = {
      ...
      Responsable1: {
        ...
        NombreyApellido: $("#ResponsableText1").val(),
      },
    ...
    var claseObj = {
      ...
      Responsable2: {
        IdEdicion: edicionActiva.idEdicion,
        NombreyApellido: $("#ResponsableText2").val(),
      },
      TituloClase: $("#Tema").val(),
      Temas: subtemas,
      IdEdicion: edicionActiva.idEdicion,
...
function listarClase(numeroPagina) {
  ...
  var fechaInicioSemana = moment(edicionActiva.fechaInicio).add(
    numeroPagina,
    "w"
  );
  var fechaFinSemana = moment(edicionActiva.fechaInicio).add(
    numeroPagina + 1,
    "w"
  );
  ...
  $.ajax({
    ...
    for (var i = 0; i < clases.length; i++) {
      ...
      html +=
        ...
        "<td>" +
        moment(clases[i].fecha).format("DD-MM-YYYY") +
        "</td>" +
        "<td>" +
        clases[i].numeroClase +
        "</td>" +
        "<td>" +
        clases[i].responsable1.nombreyApellido +
        "</td>" +
        "<td>" +
        (clases[i].responsable2
          ? clases[i].responsable2.nombreyApellido
          : "-") +
        "</td>" +
        "<td>" +
        clases[i].tituloClase +
        "</td>" +
        "<td>" +
        tema +
        "</td>" +
        '<td><button class="btn btn-light btn-editar-clase" data-id-clase="' +
        clases[i].idClase +
        '"><span class="text-primary" data-feather="edit-2"></span></button>' +
        '<button class="btn btn-light btn-enviar-clase" data-id-clase="' +
        clases[i].idClase +
        '"><span class="text-primary" data-feather="mail"></span></button>' +
        '<button class="btn btn-light btn-eliminar-clase" data-id-clase="' +
        clases[i].idClase +
        '"><span class="text-primary" data-feather="x"></span></button></td>' +
        "</tr>";
    }
...
function validateClasesEdit(idClase, callback) {
  $.ajax({
    ...
    success: function (data) {
      ...
      var IsInList =
        data.filter(function (x) {
          return x.numeroClase == numClase && x.idClase != idClase;
        }).length > 0;
...
$("#botonAgregarTemaMod").click(function () {
  if ($(".subtema-container").length <= limit) {
    var container = $(".inputClonado-editar:first").closest(
      ".subtema-container"
    );
    container.find(".errorinctema").hide();
    //se asigna a clon contoneir.clone
...
$("#table-clase").on("click", "tbody button.btn-editar-clase", function () {
  ...
  $.ajax({
    ...
    $("#ResponsableModificarText2").val(
      clase.responsable2 && clase.responsable2.nombreyApellido
    );
...
function GetPaginacion() {
  ...
  for (var i = 0; i < diferencia; i++) {
    html +=
      '<li class="page-item"><a class="page-link" onclick="listarClase(' +
      i +
      ')">' +
      (i + 1) +
      "</a></li>";
