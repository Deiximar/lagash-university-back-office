(function ($) {
  ...
  var LetterPic = function (options) {
    ...
    function getFillTypeFromSettings(settings) {
      if (!settings) return self.fillTypeGradient;
      else {
        var fill = settings.colors
          ? self.fillTypeColor
          : settings.gradients
          ? self.fillTypeGradient
          : settings.images
          ? self.fillTypeImage
          : self.fillTypeGradient;
    ...
    self.drawProviders[self.fillTypeColor] = function (settings) {
      return new BgProvider(settings, settings.colors, function (
        context,
        canvas,
        bg,
        drawTextCallback
    ...
    self.drawProviders[self.fillTypeGradient] = function (settings) {
      return new BgProvider(settings, settings.gradients, function (
        context,
        canvas,
        bg,
        drawTextCallback
    ...
    self.drawProviders[self.fillTypeImage] = function (settings) {
      return new BgProvider(settings, settings.images, function (
        context,
        canvas,
        bg,
        drawTextCallback
  ...
  $.fn.letterpic = function (options) {
    ...
    return $(
      $.map(self.$letterpics, function ($el) {
        return $el.get();
      })
    );
  };
